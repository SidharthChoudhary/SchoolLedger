...
with open('test_simplified_model.csv', 'r', encoding='utf-8') as f:
    csv_content = f.read()

# Parse as Expense
print("=" * 70)
print("PARSING EXPENSE ENTRIES (NEW SIMPLIFIED MODEL)")
print("=" * 70)
...
if result_expense['valid_rows']:
    ...
    for row_num, data in result_expense['valid_rows']:
        ...
        print(f"  Payment: {data['payment_type']}")

# Parse as Income  
print("\n" + "=" * 70)
print("PARSING INCOME ENTRIES (NEW SIMPLIFIED MODEL)")
print("=" * 70)
...
if result_income['valid_rows']:
    print("\nValid Rows:")
    for row_num, data in result_income['valid_rows']:
        print(f"\nRow {row_num}: {data['voucher_number']}")
        print(f"  Account: {data['account_name']}")
        print(f"  Amount: {data['amount']}")
